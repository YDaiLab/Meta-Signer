...
def train(train, test, config, metric, seed=42, feature_select=True):
	...
	clf = RandomForestClassifier(n_estimators=number_trees, n_jobs=-1).fit(x_filt, y)

	test_probs = np.array([row for row in clf.predict_proba(test_x_filt)])
	...
	return test_stat_dict, tpr, fpr, thresh, feature_importance, test_probs
